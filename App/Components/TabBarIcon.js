...
const TabBarIcon = ({ iconName, label, focused, additionalStyle = {} }) => {
  ...
  } else if (iconName === 'leaderboard' || iconName === 'edit') { // contoh penggunaan MaterialIcons
  ...
  return (
    <View style={{ alignItems: "center", justifyContent: "center", marginTop: 5, ...additionalStyle }}>
      <IconComponent name={iconName} size={24} color={focused ? "#BB1624" : "grey"} />
      <Text style={{ fontSize: 11, color: focused ? "#BB1624" : "grey" }}>{label}</Text>
    </View>
  );
};
