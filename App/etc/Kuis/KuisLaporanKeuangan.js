    ...
    >
      ...
      <View style={tw`flex-row mt-12 mb-2 justify-between items-center p-4`}>
        ...
        <View style={tw`bg-white py-1 px-3 rounded-lg`}>
          <Text style={tw`text-black font-bold`}>{`0:${timer < 10 ? `0${timer}` : timer}`}</Text>
        </View>
        ...
        {/* XP */}
        <View style={tw`flex-row items-center bg-[#EF980C] py-1 px-3 rounded-lg`}>
          <Image source={require('./../../assets/homePage/XP1.png')} style={tw`w-5 h-5`} />
      ...
      <ScrollView contentContainerStyle={tw`flex-grow`}>
        <View
          style={tw`bg-white min-h-full rounded-t-3xl p-6`}
        >
          {/* Pertanyaan */}
          ...
          {/* Opsi Jawaban */}
          {reactQuestionsFondationI[currentQuestionIndex].options.map((option) => (
            <Pressable
              key={option}
              style={[
          ...
          {selectedOption !== null && !isCorrect && (
            <Text style={tw`text-red-500 mt-2 text-center`}>
              Jawaban benar: {reactQuestionsFondationI[currentQuestionIndex].correctAnswer}
            </Text>
          )}
