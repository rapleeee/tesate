...
const KuisAds = ({ navigation }) => {
  ...
  const handleNext = () => {
    if (currentQuestionIndex === reactQuestionsAds.length - 1) {
      const finalScore = calculateFinalScore(reactQuestionsAds.length, mistakes);
      saveQuizScore(userUID, 'quizAdsIklan', finalScore)
        .then(() => {
          return saveTimeSpent(userUID, 'quizAdsIklan', timeElapsed);
        })
        .then(() => {
          navigation.navigate('scoreAdsIklan', { score: finalScore, timeSpent: timeElapsed });
        })
        .catch((error) => {
  ...
  const handleOptionPress = (pressedOption) => {
    ...
    const isAnswerCorrect = reactQuestionsAds[currentQuestionIndex].correctAnswer === pressedOption;
      ...
      <ScrollView style={tw`p-4`}>
        ...
        <Pressable style={tw`bg-[#BB1624] p-2 rounded-lg mt-6 mb-5`} onPress={handleNext}>
          <Text style={tw`text-white text-center text-base`}>
            {currentQuestionIndex === reactQuestionsAds.length - 1 ? 'Finish' : 'Next'}
          </Text>
        </Pressable>
      </ScrollView>
    </SafeAreaView>
